...
import type { ServiceRequestAttributes, ServiceRequestIntent, ServiceRequestPriority, ServiceRequestStatus } from '@shared/service-request'
...
function ServiceRequestForm() {
  ...
  const selectedPatient = (patients.data?.data as any[] | undefined)?.find(p => p.id === selectedPatientId)
  ...
  const onFinish = async (values: ServiceRequestFormValues) => {
    try {
      ...
      const patient = (patients.data?.data as any[] | undefined)?.find(p => p.id === values.patientId)
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <Card className="shadow-sm">
        <div className="text-center text-2xl font-bold mb-6 text-gray-700">Pemeriksaan Laboratorium</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
            ...
            <div>
              ...
              <Form.Item label="Pasien" name="patientId" rules={[{ required: true }]}>
                ...
                >
                  {(patients.data?.data as any[] | undefined)?.map((p) => (
              ...
              <Form.Item label="Umur">
                <Input readOnly value={selectedPatient?.birthDate ? `${dayjs().diff(selectedPatient.birthDate, 'year')} Tahun` : '-'} className="bg-gray-50" />
              </Form.Item>
              <Form.Item label="Alamat">
                <Input.TextArea readOnly value={selectedPatient?.addressLine || '-'} rows={2} className="bg-gray-50" />
              </Form.Item>
            </div>
            ...
            <div>
              <Form.Item label="No. Faktur">
                <Input readOnly value={isEdit ? `INV-${params.id}` : 'AUTO'} className="bg-gray-50" />
              </Form.Item>
              ...
              <Form.Item name="serviceText" hidden initialValue="General Checkup"><Input /></Form.Item>
              <Form.Item name="status" hidden initialValue="active"><Input /></Form.Item>
              <Form.Item name="intent" hidden initialValue="order"><Input /></Form.Item>
              <Form.Item name="priority" hidden initialValue="routine"><Input /></Form.Item>
            </div>
          </div>

          <div className="flex gap-2 mt-6 mb-6">
            <Button type="primary" htmlType="submit" icon={<SaveOutlined />} loading={submitting} className="bg-green-600 hover:bg-green-700 border-none">
              Selesai
            </Button>
            <Button icon={<ReloadOutlined />} onClick={() => form.resetFields()}>
              Reset
            </Button>
            <Button type="primary" danger icon={<ArrowLeftOutlined />} onClick={() => navigate('/dashboard/service-request')}>
              Kembali
            </Button>
          </div>
