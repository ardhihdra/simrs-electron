...
export default function EncounterMonitor() {
  ...
  const filteredData = data?.data?.filter(
    (item) => !filterService || (item.serviceType && item.serviceType === filterService)
  )
    ...
    <div className="flex flex-col gap-6">
      ...
      {grouped &&
        Object.entries(grouped).map(([service, items]) => (
          <div key={service}>
            <h2 className="text-2xl font-bold mb-4 capitalize border-b pb-2 dark:text-white dark:border-gray-700">
              {service}
            </h2>
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">
                    ...
                    <span className="text-lg font-bold text-blue-600">
                      {new Date(item.visitDate).toLocaleTimeString('id-ID', {
                        hour: '2-digit',
                        minute: '2-digit'
                      })}
                    </span>
      ...
      {(!grouped || Object.keys(grouped).length === 0) && (
        <div className="text-center text-gray-400 py-10">Belum ada data kunjungan</div>
      )}
    </div>
  )
}
