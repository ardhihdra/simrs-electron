...
export function DispenseReviewDetail({ open, onClose, record, employeeNameById }: Props) {
  ...
  const renderCriteria = (label: string, value?: boolean) => (
    <div className="flex items-center justify-between py-1 px-2 border-b border-gray-50 last:border-0">
      ...
      {value ? (
        <Tag
          color={desktopThemeTokens.colors.success}
          className="m-0 font-medium px-2 shadow-none border-0"
          icon={<CheckCircleFilled />}
        ></Tag>
      ) : (
        <Tag
          color={desktopThemeTokens.colors.danger}
          className="m-0 font-medium px-2 shadow-none border-0"
        ...
        <Col span={24}>
          ...
          >
            <Descriptions column={{ xs: 1, sm: 2 }} size="small" bordered>
              <Descriptions.Item label="Penyiap Obat">
                {getStaffName(record.penyiapObatId)}
              </Descriptions.Item>
              <Descriptions.Item label="Pelabel Obat">
                {getStaffName(record.pelabelObatId)}
              </Descriptions.Item>
              <Descriptions.Item label="Penyerah Obat">
                {getStaffName(record.penyerahObatId)}
              </Descriptions.Item>
              <Descriptions.Item label="Penerima Obat">
                {record.hubunganPenerima === 'Sendiri'
                  ? 'Sendiri (Pasien)'
                  : `${record.namaPenerima} (${record.hubunganPenerima})`}
              </Descriptions.Item>
            </Descriptions>
          </Card>
        </Col>
