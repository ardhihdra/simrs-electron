...
function RowActions({ record }: { record: MedicineBrandAttributes }) {
  ...
  const items: MenuProps['items'] = [
    { key: 'edit', label: 'Edit', icon: <EditOutlined />, onClick: () => typeof record.id === 'number' && navigate(`/dashboard/pharmacy/medicine-brands/edit/${record.id}`) },
    { type: 'divider' },
...
export function MedicineBrandTable() {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-4">Medicine Brands</h2>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap md:justify-end">
          ...
          <Button type="primary" onClick={() => navigate('/dashboard/pharmacy/medicine-brands/create')}>New Brand</Button>
        </div>
      </div>
