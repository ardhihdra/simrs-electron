...
import { App, Button, Card, Col, Form, Input, Row, Select, Space, Table, Tag, Divider, Typography } from 'antd'
import { useCreateNutritionOrder, useNutritionOrderByEncounter } from '../../hooks/query/use-nutrition-order'
import dayjs from 'dayjs'

const { TextArea } = Input
const { Text } = Typography

export const NutritionOrderForm = ({ encounterId, patientData }: { encounterId: string; patientData?: any }) => {
    ...
    const handleSubmit = async (values: any) => {
        try {
            const payload = {
                ...
                oralDietNutrients: values.nutrients?.map((n: string) => ({
                    code: n,
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <Space direction="vertical" size={0}>
                    {record.oralDietTypes?.map((t: any) => <Tag color="blue" key={t.id}>{t.display}</Tag>)}
                    {record.oralDietNutrients?.map((n: any) => <Tag color="orange" key={n.id}>{n.display}</Tag>)}
                </Space>
            )
        },
        {
            ...
            render: (_: any, record: any) => (
                <Space direction="vertical" size={0}>
                    {record.oralDietTextures?.map((t: any) => <Text type="secondary" key={t.id}>Tekstur: {t.display}</Text>)}
                    {record.oralDietFluids?.map((f: any) => <Text type="secondary" key={f.id}>Cairan: {f.display}</Text>)}
                </Space>
            )
        },
        {
            ...
            render: (_: any, record: any) => (
                <Space direction="vertical" size={0}>
                    {record.enteralFormula && <Tag color="purple">NGT: {record.enteralFormula.baseFormulaProductName}</Tag>}
                    {record.supplements?.map((s: any) => <Tag color="cyan" key={s.id}>{s.productName}</Tag>)}
                </Space>
            )
        },
        {
    ...
    return (
        <div className="flex flex-col gap-6">
            <Form form={form} layout="vertical" onFinish={handleSubmit}>
                <Card
                    title={<Space><CoffeeOutlined /><span>Formulir Permintaan Diet & Nutrisi (Nutrition Order)</span></Space>}
                    className="shadow-sm"
                >
                    <Row gutter={[16, 16]}>
                        <Col span={12}>
                        ...
                        <Col span={12}>
                            <Form.Item label="Kebutuhan Nutrisi Khusus" name="nutrients">
                                <Select mode="multiple" options={NUTRIENTS} placeholder="Pilih jika ada (bisa lebih dari satu)..." />
                            </Form.Item>
                        </Col>
                    </Row>
                    ...
                    <Row gutter={[16, 16]}>
                        <Col span={12}>
                            <Form.Item label="Formula Enteral (Via NGT/OGT)" name="enteralFormula" tooltip="Diisi jika pasien menggunakan selang makan">
                    ...
                    <Form.Item label="Instruksi Khusus / Catatan Dietisien" name="instruction">
                        <TextArea rows={3} placeholder="Contoh: Pasien alergi udang, mohon makanan disajikan hangat." />
                    </Form.Item>
            ...
            <Card
                title={<Space><HistoryOutlined /><span>Riwayat Order Diet</span></Space>}
                className="shadow-sm"
            >
                <Table
