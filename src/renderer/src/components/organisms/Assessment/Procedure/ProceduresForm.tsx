...
export const ProceduresForm = ({ encounterId, patientData }: ProceduresFormProps) => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (response.success) {
        message.success('Tindakan medis berhasil disimpan')
  ...
  if (isLoadingProcedures) {
    return (
      <div className="flex justify-center items-center h-48">
        <Spin size="large" tip="Memuat data tindakan..." />
      </div>
    )
  }
