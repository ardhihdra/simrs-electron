...
export const AnamnesisForm = ({ encounterId, patientData }: AnamnesisFormProps) => {
    ...
    const { data: performersData, isLoading: isLoadingPerformers } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (res.success && res.result) {
                return res.result
                    .filter((p: any) => p.hakAksesId === 'nurse' || p.hakAksesId === 'doctor') // Allow both? Or just doctor? Usually anamnesis is by doctor.
                    .map((p: any) => ({
                        id: p.id,
                        name: p.namaLengkap
                    }))
    ...
    const handleFinish = async (values: any) => {
        ...
        try {
            ...
            // Note: Anamnesis data (conditions) doesn't strictly adhere to observation effectiveDateTime 
            // in the same way, but it's good practice. Currently bulkCreateCondition doesn't accept date override easily 
            // unless we modify the hook, but let's stick to existing logic.
            ...
            if (conditions.length > 0) {
                promises.push(
                    bulkCreateCondition.mutateAsync({
                        encounterId,
                        patientId,
                        doctorId: 0, // Should be dynamic if possible, but 0 is standard fallback here
                        conditions
                    })
                )
            }
            ...
            if (anamnesis?.familyHistoryList && anamnesis.familyHistoryList.length > 0) {
                promises.push(createFamilyHistory.mutateAsync(
                    buildFamilyHistory({
                        patientId,
                        status: 'completed',
                        relationship: 'other',
            ...
            message.error(`Gagal menyimpan anamnesis: ${error?.message || 'Error'}`)
        } finally {
