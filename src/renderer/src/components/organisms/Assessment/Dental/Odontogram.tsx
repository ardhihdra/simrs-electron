...
import { convertFDIToNotation, getToothNotations, mapToCssVars } from './dental-helpers'
...
export const Odontogram: FC<OdontogramProps> = ({
  ...
  const handleHover = (
    ...
    const { x, y } = placements[placement]?.(toothBox, margin) ?? placements.right(toothBox, margin)
    ...
    setTooltipData({
      ...
      payload: {
        ...
        type:
          teethPaths.find((t) => t.name === name.replace('teeth-', '').slice(1))?.type ?? 'Unknown'
      }
    })
  }
  ...
  return (
    <div
      className={`Odontogram ${theme === 'dark' ? 'dark-theme' : ''} ${className}`}
      style={{
    ...
    >
      <input type="hidden" name={name ?? 'teeth'} value={JSON.stringify(Array.from(selected))} />
      <svg
        ref={svgRef}
      ...
      >
        ...
        {visibleQuadrants.map(({ name, transform, label }, index) => (
          <g role="group" aria-label={label} key={name} name={name} transform={transform}>
