...
import {
  ...
  PSYCHOLOGICAL_STATUS_SNOMED_MAP
} from '@renderer/config/maps/observation-maps'
  ...
  useEffect(() => {
    ...
    if (response?.success && observations && observations.length > 0) {
      ...
      if (mode === 'inpatient') {
        ...
        // Psychosocial
        form.setFieldsValue({
          psychological_status: Array.isArray(psychosocialHistory.psychological_status)
            ? psychosocialHistory.psychological_status
            : psychosocialHistory.psychological_status?.split(', '),
