...
const mapEncounterStatus = (status?: string | null): PatientStatus => {
  switch (status) {
    ...
    case 'triaged':
      return PatientStatus.WAITING
    case 'in-progress':
      return PatientStatus.EXAMINING
    case 'finished':
...
export const getPatientMedicalRecord = async (
  ...
  try {
    ...
    if (observations || conditions.length > 0) {
      ...
      const vitalSigns = {
        systolicBloodPressure: getNumberVal(observations?.vitalSigns || [], [
          'systolic-blood-pressure',
          '8480-6'
        ]),
        diastolicBloodPressure: getNumberVal(observations?.vitalSigns || [], [
          'diastolic-blood-pressure',
          '8462-4'
        ]),
        heartRate: getNumberVal(observations?.vitalSigns || [], ['heart-rate', '8867-4']),
        pulseRate: getNumberVal(observations?.vitalSigns || [], [
          'pulse-rate',
          'heart-rate',
          '8867-4'
        ]),
        respiratoryRate: getNumberVal(observations?.vitalSigns || [], [
          'respiratory-rate',
          '9279-1'
        ]),
        temperature: getNumberVal(observations?.vitalSigns || [], ['body-temperature', '8310-5']),
        oxygenSaturation: getNumberVal(observations?.vitalSigns || [], [
          'oxygen-saturation',
          '59408-5',
          '59408-5'
        ]),
      ...
      const anamnesis = {
        ...
        historyOfPresentIllness:
          ...
          getStringVal(observations?.anamnesis || [], [
      ...
      const physicalExamination = {
        ...
        additionalNotes: getStringVal(observations?.physicalExam || [], [
          'additional-notes',
          'physical-notes'
        ])
      }
      ...
      nurseRecord = {
        ...
        anamnesis,
        physicalExamination,
        examinationDate: encounter.period?.start
...
export const saveDiagnosisAndProcedures = async (
  ...
  try {
    ...
    if (notesToSave.length > 0) {
      ...
      if (!noteRes.success) {
        const errorMsg =
          noteRes.message || (noteRes as any).error || 'Failed to save clinical/referral notes'
  ...
  try {
    ...
    if (response?.success && Array.isArray(response.result)) {
      let medicines = (response.result as BackendMedicine[]).map((m) => ({
        ...
        stock: 100,
        price:
          typeof m.sellingPrice === 'string' ? parseFloat(m.sellingPrice) : m.sellingPrice || 0,
...
export const getPrescriptionByEncounter = async (
  encounterId: string
