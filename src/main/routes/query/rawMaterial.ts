...
export const list = async (ctx: IpcContext) => {
  ...
  const res = await client.get('/api/substance?items=100&depth=1')
...
export const read = async (ctx: IpcContext, args: z.infer<typeof schemas.read.args>) => {
  const client = createBackendClient(ctx)
  const res = await client.get(`/api/substance/read/${args.id}`)
...
export const create = async (ctx: IpcContext, args: z.infer<typeof schemas.create.args>) => {
  ...
  const res = await client.post('/api/substance', payload)
  const result = await parseBackendResponse(res, BackendDetailSchema)
  return { success: true, result }
}
...
export const update = async (ctx: IpcContext, args: z.infer<typeof schemas.update.args>) => {
  ...
  const res = await client.put(`/api/substance/${args.id}`, payload)
  const result = await parseBackendResponse(res, BackendDetailSchema)
  return { success: true, result }
}
...
export const deleteById = async (ctx: IpcContext, args: z.infer<typeof schemas.deleteById.args>) => {
  const client = createBackendClient(ctx)
  const res = await client.delete(`/api/substance/${args.id}`)
