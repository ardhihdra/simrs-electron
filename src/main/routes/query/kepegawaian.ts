...
export const list = async (ctx: IpcContext, args?: z.infer<typeof schemas.list.args>) => {
  try {
    ...
    const result = await parseBackendResponse(res, BackendListSchema(KepegawaianSchemaWithId))
...
const defaultcrud = createCrudRoutes({
  entity: 'kepegawaian',
  schema: z.any()
})

export const { listAll } = defaultcrud
