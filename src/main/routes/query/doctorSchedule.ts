import z from 'zod'
import { DoctorScheduleSchema, DoctorScheduleSchemaWithId } from '@main/models/doctorSchedule'
...
export const schemas = {
  list: {
    result: z.object({
      success: z.boolean(),
      result: DoctorScheduleSchemaWithId.array().optional(),
  ...
  getById: {
    ...
    result: z.object({
      success: z.boolean(),
      result: DoctorScheduleSchemaWithId.optional(),
      message: z.string().optional()
    })
  },
  create: {
    args: DoctorScheduleSchema.partial(),
    result: z.object({
      success: z.boolean(),
      result: DoctorScheduleSchemaWithId.optional(),
      message: z.string().optional()
  ...
  update: {
    args: DoctorScheduleSchemaWithId,
    result: z.object({
      success: z.boolean(),
      result: DoctorScheduleSchemaWithId.optional(),
      message: z.string().optional()
...
export const list = async (ctx: IpcContext) => {
  try {
    ...
    const res = await client.get('/api/jadwalDokter?items=100')
    const result = await parseBackendResponse(
      res,
      BackendListSchema(DoctorScheduleSchemaWithId)
    )
...
export const getById = async (ctx: IpcContext, args: z.infer<typeof schemas.getById.args>) => {
  try {
    const client = createBackendClient(ctx)
    const res = await client.get(`/api/jadwalDokter/read/${args.id}`)
...
export const deleteById = async (ctx: IpcContext, args: z.infer<typeof schemas.deleteById.args>) => {
  try {
    const client = createBackendClient(ctx)
    const res = await client.delete(`/api/jadwalDokter/${args.id}`)
